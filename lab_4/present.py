...
def genRoundKeys(key):
    ...
    for round in range(0, FULLROUND + 1):
        round_keys.append(key >> 16) # add round key
        key = rol(key, 61, 80)
        key = ((sbox[key >> 76] & 0xF) << 76) | (key & (2**76 - 1)) # passing [79..76] through S-box
        key ^= (round + 1) << 15
    return round_keys
...
def addRoundKey(state, Ki):
    return state^Ki # for last key at end for post-whitening


def sBoxLayer(state):
    output = 0
    for i in range(16):
        output |= sbox[(state >> (4 * i)) & 0xF] << (4 * i) # substituting words according to lookup table
    return output

def sBoxLayerInv(state):
    output = 0
    for i in range(16):
        output |= sbox.index((state >> (4 * i)) & 0xF) << (4 * i)
    return output

def pLayer(state):
    output = 0
    # moving bits around (permutation)
    for i in range(64):
        bit = (state >> i) & 0x1
        output |= bit << pmt[i]
    return output
